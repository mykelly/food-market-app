...
import com.food.market.listener.HomeViewFlowListener;
...
public class HomeViewFlow extends AdapterView<Adapter> {
    ...
    private final static int DEFAULT_HEIGHT = ScreenUtil.dip2px(203);
    ...
    private boolean canTouch = true;// 是否允许停止播放，默认状态true表示允许停止播放
    public static boolean onTouch = false;// 是否点击到了viewFlow的区域，用于自定义图层中重定向事件的传递。
    public OnClickPosition onclick;

    private MySwipeRefreshLayout disAllowTouchSwipeView;
    private ScrollView disAllowTouchScrollView;

    private Context mContext;

    private HomeViewFlowListener listener;
    ...
    private void init(Context context) {
        ...
        onclick = new OnClickPosition() {

            @Override
            public void onClickPosition() {
                // TODO Auto-generated method stub
//				int position=getSelectedItemPosition()%getCurrentScreen();
    ...
    public boolean isStop() {
        if (handler != null) {
            return !handler.hasMessages(0) ? true : false;
        } else {
            return true;
        }
    }
     ...
     * 是否允许停止播放
     *
     * @param allow false:不允许暂停 true:可以暂停
     */
    public void canTouchStop(boolean allow) {
        this.canTouch = allow;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode != MeasureSpec.EXACTLY && !isInEditMode()) {
            /*throw new IllegalStateException(
					"ViewFlow can only be used in EXACTLY mode.");*/
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(ScreenUtil.dip2px(DEFAULT_HEIGHT), MeasureSpec.EXACTLY);
            widthMeasureSpec = MeasureSpec.makeMeasureSpec(ScreenUtil.getScreenWidthPixel((Activity) mContext), MeasureSpec.EXACTLY);
            super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (disAllowTouchScrollView != null){
                    disAllowTouchScrollView.requestDisallowInterceptTouchEvent(true);
                }
                if (disAllowTouchSwipeView != null) {
                    Log.d("tag", "the swipe enable down is======" + String.valueOf(disAllowTouchSwipeView.isEnabled()));
                    disAllowTouchSwipeView.setEnabled(false);
                }
                if (viewPager != null) {
                    viewPager.requestDisallowInterceptTouchEvent(true);
//				listener.ViewFlowListener(false);
                }
			/*
...
//				listener.ViewFlowListener(false);
            ...
            case MotionEvent.ACTION_UP:
                if (disAllowTouchScrollView != null){
                    disAllowTouchScrollView.requestDisallowInterceptTouchEvent(false);
                }
                if (disAllowTouchSwipeView != null) {
                    disAllowTouchSwipeView.setEnabled(true);
                }
                if (viewPager != null) {
        ...
        switch (action) {
			 ...
			 */
                ...
                if (disAllowTouchSwipeView != null) {
                    Log.d("tag", "the swipe enable down is======" + String.valueOf(disAllowTouchSwipeView.isEnabled()));
                    disAllowTouchSwipeView.setEnabled(false);
                    disAllowTouchSwipeView.requestDisallowInterceptTouchEvent(true);
                }
                if (viewPager != null) {
                    viewPager.requestDisallowInterceptTouchEvent(true);
//				listener.ViewFlowListener(false);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (disAllowTouchSwipeView != null) {
                    Log.d("tag", "the swipe enable down is TOUCH======" + String.valueOf(disAllowTouchSwipeView.isEnabled()));
                    disAllowTouchSwipeView.setEnabled(false);
                    disAllowTouchSwipeView.requestDisallowInterceptTouchEvent(true);
                }
                if (viewPager != null) {
...
//				listener.ViewFlowListener(false);
            ...
            case MotionEvent.ACTION_UP:
              /*  if (disAllowTouchScrollView != null){
                ...
                if (disAllowTouchSwipeView != null) {
                    TTLog.d("tag", "the swipe enable down is======" + String.valueOf(disAllowTouchSwipeView.isEnabled()));
                    disAllowTouchSwipeView.setEnabled(true);
                }
                if (viewPager != null) {
                    viewPager.requestDisallowInterceptTouchEvent(false);
...
//				listener.ViewFlowListener(false);
                ...
                // 有可能终止，重新发延迟消息以恢复自动播放，如果已存在该消息则不发送
                if (handler != null && !handler.hasMessages(0)) {
                    Message message = handler.obtainMessage(0);
                    handler.sendMessageDelayed(message, timeSpan);
                }
    ...
    /**
     * 按下返回键回调用这个办法 而Home键是不会调用的 有可能是会呈现异常，扭转屏幕时会先detach再attach
     */
    @Override
    ...
    public void setListener(HomeViewFlowListener listener) {
	...
	public void setScrollView(ScrollView scrollView){
        this.disAllowTouchScrollView = scrollView;
    }

    public void setSwipeLayout(MySwipeRefreshLayout view) {
        disAllowTouchSwipeView = view;
    }
